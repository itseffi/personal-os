...
Copy this to `.agents/skills/setup-granola/SKILL.md` for canonical runtime skills.
If needed for Claude discovery, the repo bridge `.claude/skills -> ../.agents/skills` will expose it.
...
Copy the Granola skills to the canonical skills folder:

```bash
mkdir -p .agents/skills/granola-meeting-sync
cp System/integrations/granola/skills/meeting-sync/SKILL.md .agents/skills/granola-meeting-sync/SKILL.md
```
...
Ask user to restart their agent session, then test:
...
- [ ] meeting-sync skill installed to .agents/skills/
- [ ] .mcp.json updated with granola config
- [ ] Agent session restarted
...
- Daily planning will automatically check for new meetings
